...
class ConcreteSarsaSpec
  ...
  "learnN shouldn't overflow stack (learnN is tailrec, each episode tailrec)" in {
    ...
    val result: Randomized[(sarsa.vf.Q, List[List[Double]])] =
      sarsa.learn (sarsa.vf.initialize, List[List[Double]](), initials.take (C))
    try result.size == 1
    catch case e =>
  ...
  "learn is tail recursive, no stack overflow (regression)"  in {
    val result = sarsa.learningEpisode ((sarsa.vf.initialize, List[List[Double]]()), ())
    result.head
  }
