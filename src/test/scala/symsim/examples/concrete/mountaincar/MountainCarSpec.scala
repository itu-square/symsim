...
class MountainCarSpec
  ...
  "Sanity checks for symsim.concrete.mountaincar" - {
    ...
    "Achieving the goal state from state (0.0, 0.0) with a single action is impossible" in check {
      forAll (actions) { a =>
    ...
    "The car cannot move beyond -1.2 on the left" in check {
      val dt: Double = 0.1
      val mass: Double = 0.2
    ...
    "The car can move to the position -1.2" in check {
      val (s1, r) = MountainCar.step (
        CarState (-1.1601645563843357, -1.1533417842134712)) (-0.2).head
      s1.p == -1.2
    }

    "gravity and friction are both decreasing the velocity" in check {
